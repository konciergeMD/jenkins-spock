...
//					.filterInputsBy(new FilterBuilder().excludePackage("org.bouncycastle"))
        ...
        try {
            all_types = reflector.getAll(SubTypes);
    ...
    public Set<Class<?>> getClassesWithAnnotationOfTypeInPackage(Class<? extends Annotation> _annotation, Class<?> _supertype, Optional<String> _package) {
        ...
        for (Class<?> annotated_class : new Reflections(_package.orElse("")).get(SubTypes.of(TypesAnnotated.with(_annotation)).asClass())) {
